...
lista <- asplit(community,1)
lista1 <- Filter(function(x)sum(x)!= 0, lista)
lista1 <- lapply(lista1, as.vector)
out1 <- iNEXT(lista1,q = 0,
              datatype = "abundance")
out <- out1
...
all_rich <- all_rich %>% rename(nome_uc = Assemblage)

all_rich1 <- merge(all_rich, UCs_sf, by = "nome_uc", all.x = TRUE )
